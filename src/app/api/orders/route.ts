...
const getServiceClient = () => getSupabaseServiceClient();
...
export async function GET(request: Request) {
  try {
    ...
    // Build base query with order items
    let query = supabase
      .from('orders')
    ...
    // Permission filter: super_admin sees all, others see their own
    ...
    // Get status statistics for the user
...
// POST /api/orders - Create order with items (transaction-like)
export async function POST(request: Request) {
  try {
    ...
    const { order_no, customer_id, customer_name, delivery_date, remark, items } = body;
...
export async function PUT(request: Request) {
  try {
    ...
    const validStatuses = ['pending', 'returned', 'confirmed', 'pool', 'producing', 'shipped', 'completed', 'cancelled'];
